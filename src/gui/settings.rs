...
pub fn cache_dir_management(ui: &mut egui::Ui, locale: &FluentBundle<Arc<FluentResource>>) {
    egui::Frame::group(ui.style()).show(ui, |ui| {
        ui.heading(locale::get_message(locale, "paths", None));
...
pub fn language(ui: &mut egui::Ui, locale: &FluentBundle<Arc<FluentResource>>) -> bool {
    egui::Frame::group(ui.style())
        .show(ui, |ui| {
            ...
            config::set_config_value("language_search", language_search.clone().into());

            let languages = locale::get_language_list();
            let filtered: Vec<_> = if language_search.is_empty() {
                languages
            } else {
                languages
                    .into_iter()
                    .filter(|l| {
                        l.0.to_lowercase().contains(&language_search.to_lowercase())
                            || l.1.to_lowercase().contains(&language_search.to_lowercase())
                    })
                    .collect()
            };
